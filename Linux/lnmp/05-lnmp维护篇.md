...
1. Nginx 主进程用户
   ...
   Nginx 主进程用户为登陆用户，一般都是 root 用户执行（所以 nginx 配置文件所有者都应该是 root），这里不做过多讲解！
...
3. php-fpm 主进程用户
   ...
   php-fpm 主进程用户为登录用户，一般都是 root 用户执行（所以 php-fpm 配置文件所有者都应该是 root），这里不做过多讲解！
...
为了是服务器更加安全，我们应该遵循如下操作准则：

| 序号 | 安全准则                                                             |
| ---- | -------------------------------------------------------------------- |
| 01   | 工作进程尽量使用非特权用户运行，减低脚本漏洞对服务器的伤害           |
| 02   | 配置文件所属用户使用 root，减低被篡改的可能                          |
| 03   | ftp 应该限制 IP 登陆，保证 ftp 仅自己人登陆                          |
| 04   | `nginx用户` 对 `php/网页文件` 通常只需要可读权限                     |
| 05   | `php-fpm用户` 对 `php/网页文件` 通常只需要可读权限                   |
| 06   | 重要的 `php/网页文件` 所属用户尽可能使用 root 用户，减低被篡改的可能 |

为了便于维护，我们一般都会对服务器做这些危害事件：

| 序号 | 危害事件                                                  |
| ---- | --------------------------------------------------------- |
| 01   | `php文件/网页文件` 所属用户为 `ftp用户`                   |
| 02   | `ftp用户` 对 `php文件/网页文件` 拥有写的权限              |
| 03   | `php-fpm/nginx用户` 被迫加入了 `ftp用户组`                |
| 04   | 假如 `ftp用户` 泄露，黑客就能随意操作 nginx、php-fpm 文件 |
